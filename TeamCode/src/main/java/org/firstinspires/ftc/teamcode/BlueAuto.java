...
public class BlueAuto extends OpMode {
    ...
    double time = kickTimer.milliseconds();
    ...
    private final Pose scorePose = new Pose(12, -28, Math.toRadians(45)); // Scoring Pose of our robot. It is facing the goal at a 135 degree angle.
    private final Pose pickup1Pose = new Pose(12, -58.6, Math.toRadians(0)); // Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose pickup1EndPose = new Pose(-26, -58.6, Math.toRadians(0)); // Middle (Second Set) of Artifacts from the Spike Mark.
    private final Pose pickup2StartPose = new Pose(12, -33, Math.toRadians(0)); // Lowest (Third Set) of Artifacts from the Spike Mark.
    private final Pose pickup2EndPose = new Pose(-16, -33, Math.toRadians(0));
    private final Pose pickup3StartPose = new Pose(12, -87, Math.toRadians(0));
    private final Pose pickup3EndPose = new Pose(-26, -87, Math.toRadians(0));
    private final Pose end = new Pose(12,-12,Math.toRadians(29));

    private Path scorePreload;
    private PathChain grab1,grabend1,score1,grab2,grabend2,score2,grab3,grabend3,endfr;
    ...
    scorePreload.setConstantInterpolation(startPose.getHeading()); */
        ...
        endfr = follower.pathBuilder()
                .addPath(new BezierLine(pickup3EndPose, end))
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 102: // The Shooting Sequence
                ...
                if (pathTimer.getElapsedTime() > 999) {
                    kicker.setPosition(0.38);
                }

                // After 2 seconds total, reset and move to next path
                if (pathTimer.getElapsedTime() > 1670) {
                    left.setPosition(0.65);
                    right.setPosition(0.8);
                    intake.setPower(0);
                    kicker.setPosition(0);
            ...
            */
            ...
            case 3:
                ...
                if (follower.isBusy()) {
                    ...
                    setPathState(31); // Move to a "wait for score" state
                }
                break;
            ...
            case 9:
                ...
                if (follower.isBusy()) {
                    ...
                    /* Since this is a pathChain, we can have Pedro hold the end point while we are scoring the sample */
                    intake.setPower(0.0);
                    follower.followPath(endfr,true);
                    setPathState(91);
                }
                break;
